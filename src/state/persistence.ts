...
export async function projectFromTemplate(templateId: string, newId: string): Promise<Project | null> {
  ...
  if (!rec) return null
  try {
    return { ...migrateProject(rec.json), id: newId, name: `${rec.name}` }
  } catch {
    return null
  }
}
...
export async function loadProjectRecord(id: string): Promise<Project | null> {
  ...
  if (!rec?.json) return null
  try {
    return migrateProject(rec.json)
  } catch {
    return null // unreadable record — better an empty editor than a crash loop
  }
}
...
export async function loadProjectFile(file: File): Promise<Project> {
  ...
  // migrate = parse + repair; media rehydrates from IndexedDB by asset id,
  // or the Media panel re-links by filename.
